...
def plot(dname):
    ...
    f = ats_xdmf.VisFile(dname, time_unit='s')
    f.loadMesh(order=['x',])
    ...
    for i, (comp, cmap) in enumerate(zip(comps, cmaps)):
        ax, _ = f.plotLinesInTime(f"total_component_concentration.{comp}",
                                  colorbar_label=f"{comp} in time [s]", colorbar_ticks=(i == 0), cmap=cmap, ax=ax)
    axs[0].set_ylabel("concentration [mol / mol H2O]")

    ax = axs[1]
    for i, (comp, cmap) in enumerate(zip(comps, cmaps)):
        ax, _ = f.plotLinesInTime(f"total_sorbed.{comp}",
                                  colorbar_label=f"{comp} in time [s]", colorbar_ticks=(i == 0), cmap=cmap, ax=ax)
    ax.set_ylabel("total sorbed")
