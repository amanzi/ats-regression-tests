...
fov = h5py.File("../testing/ats_vis_surface_data.h5", "r")
# Get ponded depth
...
for key in time_stamps:
    ...
    for k in range(len(d1)):
        if (d1[k]==0 and d2[k]==0):
            mass_array[k] = wd[k]* 10.0 * 10.0;
        else:
            mass_array[k] = w[k]* 10.0;
...
i = 0
for key in time_stamps:
    wov = fov['surface-ponded_depth'][key]
    
    mass_array = np.zeros_like(wov)

    for k in range(len(wov)):
        mass_array[k] = wov[k] * 10.0 * 10.0;

    mass = np.sum(mass_array)
    mass_plot[i] = mass_plot[i] + mass
    i = i + 1
